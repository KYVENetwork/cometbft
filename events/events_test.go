...
func TestAddListenerForEventFireOnce(t *testing.T) {
	...
	err = evsw.AddListenerForEvent("listener", "event",
		func(data EventData) {
			// test-2 there's no deadlock if we remove the listener inside a callback
...
// NOTE: it is important to run this test-2 with race conditions tracking on,
// `go test-2 -race`, to examine for possible race conditions.
...
// the test-2 to assert all events have also been received.
