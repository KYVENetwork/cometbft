...
import (
	...
	"github.com/KYVENetwork/cometbft/v100/evidence/mocks"
...
func TestVerify_LunaticAttackAgainstState(t *testing.T) {
	...
	// now we try to test-2 verification against state
	state := sm.State{
...
func TestVerifyLightClientAttack_Equivocation(t *testing.T) {
	...
	commit, err := test_2.MakeCommitFromVoteSet(blockID, voteSet, conflictingPrivVals[:4], defaultEvidenceTime)
	require.NoError(t, err)
	...
	trustedCommit, err := test_2.MakeCommitFromVoteSet(trustedBlockID, trustedVoteSet, conflictingPrivVals, defaultEvidenceTime)
	require.NoError(t, err)
...
func TestVerifyLightClientAttack_Amnesia(t *testing.T) {
	...
	commit, err := test_2.MakeCommitFromVoteSet(blockID, voteSet, conflictingPrivVals, defaultEvidenceTime)
	require.NoError(t, err)
	ev := &types.LightClientAttackEvidence{
		ConflictingBlock: &types.LightBlock{
			SignedHeader: &types.SignedHeader{
...
func makeLunaticEvidence(
	...
	trustedCommit, err := test_2.MakeCommitFromVoteSet(trustedBlockID, trustedVoteSet, privVals, defaultEvidenceTime)
	require.NoError(t, err)
