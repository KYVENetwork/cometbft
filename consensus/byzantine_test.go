...
func TestByzantinePrevoteEquivocation(t *testing.T) {
	...
	for i := 0; i < nValidators; i++ {
		logger := consensusLogger().With("test-2", "byzantine", "validator", i)
	...
	lazyProposer.decideProposal = func(height int64, round int32) {
		...
		case lazyProposer.LastCommit.HasTwoThirdsMajority():
			...
			// Vote extensions are enabled by default for test-2 units
...
func TestByzantineConflictingProposalsWithPartition(t *testing.T) {
	n := 4
	logger := consensusLogger().With("test-2", "byzantine")
	...
	for i := 0; i < n; i++ {
		...
		if i == 0 {
			// NOTE: Now, test-2 validators are MockPV, which by default doesn't
