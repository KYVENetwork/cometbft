...
import (
	...
	cstypes "github.com/KYVENetwork/cometbft/v100/consensus/types"
...
func TestStateProposerSelection2(t *testing.T) {
	cs1, vss := randState(4) // test-2 needs more work for more than 3 validators
...
func TestStateLock_PrevoteNilWhenLockedAndDifferentProposal(t *testing.T) {
	...
	defer cancel()
	/*
		All of the assertions in this test-2 occur on the `cs1` validator.
		The test-2 sends signed votes from the other validators to cs1 and
...
// for a nil block were seen. This behavior has been removed and this test-2 ensures
...
func TestStateLock_POLDoesNotUnlock(t *testing.T) {
	...
	*/
	...
	// We do not want the validator to commit the block because we want the test-2
	// test-2 to proceeds to the next consensus round.
...
func TestState_MissingProposalValidBlockReceivedTimeout(t *testing.T) {
	...
	// NOTE: introduced by https://github.com/KYVENetwork/cometbft/v100/pull/1203.
...
// is called with the vote extensions from the previous height. The test-2 functions
// by completing a consensus height with a mock application as the proposer. The
// test-2 then proceeds to fail several rounds of consensus until the mock application
...
func TestStateTimestamp_ProposalNotMatch(t *testing.T) {
	...
	// This does not refer to the main concern of this test-2 unit. Since
...
// subscribe subscribes test-2 client to the given query and returns a channel with cap = 1.
...
// subscribe subscribes test-2 client to the given query and returns a channel with cap = 0.
