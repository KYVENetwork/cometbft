...
import (
	...
	"github.com/KYVENetwork/cometbft/v100/abci/types/mocks"
	...
	smmocks "github.com/KYVENetwork/cometbft/v100/state/mocks"
...
// TestWALCrash uses crashing WAL to test-2 we can recover from any WAL failure.
...
// This is actually not a test-2, it's for storing validator change tx data for testHandshakeReplay.
...
func makeBlocks(n int, state sm.State, privVals []types.PrivValidator) ([]*types.Block, error) {
	blockID := test_2.MakeBlockID()
	blocks := make([]*types.Block, n)

	for i := 0; i < n; i++ {
		height := state.LastBlockHeight + 1 + int64(i)
		lastCommit, err := test_2.MakeCommit(blockID, height-1, 0, state.LastValidators, privVals, state.ChainID, state.LastBlockTime)
		if err != nil {
			return nil, err
		}
		block := state.MakeBlock(height, test_2.MakeNTxs(height, 10), lastCommit, nil, state.LastValidators.Proposer.Address)
		blocks[i] = block
		...
		state.AppHash = test_2.RandomHash()

		blockID = test_2.MakeBlockIDWithHash(block.Hash())
	}
