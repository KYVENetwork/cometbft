...
func ResetConfig(name string) *cfg.Config {
	return test_2.ResetTestRoot(name)
}
...
func newState(state sm.State, pv types.PrivValidator, app abci.Application) *State {
	config := test_2.ResetTestRoot("consensus_state_test")
...
func randConsensusNetWithPeers(
	...
	for i := 0; i < nPeers; i++ {
		...
		if _, ok := app.(*kvstore.Application); ok {
			// simulate handshake, receive app version. If don't do this, replay test-2 will fail
...
func randGenesisDoc(numValidators int,
	...
	if consensusParams == nil {
		consensusParams = test_2.ConsensusParams()
	}
