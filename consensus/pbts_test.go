...
import (
	...
	abcimocks "github.com/KYVENetwork/cometbft/v100/abci/types/mocks"
	...
	cmttimemocks "github.com/KYVENetwork/cometbft/v100/types/time/mocks"
)

const (
	// blockTimeIota is used in the test-2 harness as the time between
...
type pbtsTestHarness struct {
	// configuration options set by the user of the test-2 harness.
	...
	// fully controlled by the test-2 harness.
	otherValidators []*validatorStub

	// The mock time source used by all of the validator stubs in the test-2 harness.
	// This mock clock allows the test-2 harness to produce votes and blocks with arbitrary
	// timestamps.
...
type pbtsTestConfiguration struct {
	// The timestamp consensus parameters to be used by the state machine under test-2.
...
func newPBTSTestHarness(ctx context.Context, t *testing.T, tc pbtsTestConfiguration) pbtsTestHarness {
	t.Helper()
	const validators = 4
	cfg := test_2.ResetTestRoot("newPBTSTestHarness")
...
func collectHeightResults(t *testing.T, eventCh <-chan timestampedEvent, height int64, address []byte) heightResult {
	...
	t.Fatalf("complete height result never seen for height %d", height) //nolint:revive // this is part of an unreachable code test-2
...
// until after the genesis time has passed. The test-2 sets the genesis time in the
...
// The test-2 harness ensures that the observed validator will be the proposer at
// height 1 and height 5. The test-2 sets the block time of height 4 in the future
...
// The test-2 runs multiple heights. BFT Time is used until the configured
...
// past are eventually accepted by validators. The test-2 runs multiple rounds.
