...
func destroyTestGroup(t *testing.T, g *Group) {
	...
	require.NoError(t, err, "Error removing test-2 Group directory")
}
...
// test-2 that Read reads the required amount of bytes from all the files in the
...
// test-2 that Read returns an error if number of bytes read < size of
