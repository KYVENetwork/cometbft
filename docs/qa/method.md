...
[releases]: https://github.com/KYVENetwork/cometbft/v37/blob/v0.37.x/RELEASES.md#large-scale-testnets
...
2. Extract the latency report and the raw latencies for all the experiments. Run these commands from the directory containing the blockstore
    * ```bash
       mkdir results
       go run github.com/KYVENetwork/cometbft/v37/test/loadtime/cmd/report@f1aaa436d --database-type goleveldb --data-dir ./ > results/report.txt`
       go run github.com/KYVENetwork/cometbft/v37/test/loadtime/cmd/report@f1aaa436d --database-type goleveldb --data-dir ./ --csv results/raw.csv`
       ```
